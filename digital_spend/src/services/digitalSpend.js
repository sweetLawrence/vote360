...
// Called by POST /api/v1/digital/record
async function addDigitalSpendRecord(body) {
  const errors = [];
  ...
  // Resolve spend amount — accept direct amount or range
  let spendAmount = null;
  if (body.spend_amount && !isNaN(Number(body.spend_amount))) {
    spendAmount = Number(body.spend_amount);
  } else {
    spendAmount = normalizeMidpoint(body.spend_range_low, body.spend_range_high, body.spend_range);
  }
  if (!spendAmount || spendAmount <= 0) {
    errors.push('provide spend_amount, or spend_range_low + spend_range_high, or spend_range');
  }

  if (!body.period_start) errors.push('period_start is required (YYYY-MM-DD)');
  if (!body.period_end) errors.push('period_end is required (YYYY-MM-DD)');
  ...
  const row = {
    ...
    platform,
    spend_amount: spendAmount,
    spend_range_low: body.spend_range_low ? Number(body.spend_range_low) : null,
    spend_range_high: body.spend_range_high ? Number(body.spend_range_high) : null,
  ...
  return {
    success: true,
    message: 'Digital spend record added and reconciliation triggered',
    record: data,
  };
}

module.exports = { importDigitalSpend, getDigitalSpend, addDigitalSpendRecord };